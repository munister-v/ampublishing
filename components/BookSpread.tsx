  ...
  return (
    <>
      ...
      >
        ...
        >
          {body}
          {/* Считаем только страницы самого отрывка: метка стоит до концовки,
              поэтому служебный финал не добавляет разворотов. */}
          <span ref={endRef} aria-hidden className="inline-block h-px w-px" />
          {/* Текст кончился на левой странице — правая осталась бы пустой.
              Раньше туда ставилась пустая заглушка (она нужна, чтобы последний
              разворот доезжал до места), и читатель видел чистый лист. Теперь
              на этой странице стоит концовка: пустой страницы не остаётся, а
              разворот по-прежнему встаёт ровно. Так закрывают главу и в книге —
              виньеткой на добавочной полосе. */}
          {oddTail && (
            <div className="flex h-full flex-col justify-center" style={{ breakBefore: 'column' }}>
              {footer}
            </div>
          )}
        </div>
      ...
      {/* Под разворотом концовка нужна только тогда, когда текст занял все
          колонки без остатка: в нечётном случае она уже стоит на добавочной
          странице, и второй раз её показывать нельзя. */}
      {!oddTail && page === pages - 1 ? footer : null}
